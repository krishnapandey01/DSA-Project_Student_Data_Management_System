...
struct Student makeStudent(char fname[], char lname[], int rollNo, char hNumber[], char city[], char state[], char email[],
...
int main()
{
  ...
  //stuSearch(head);
  /////////////////////////
  //struct Student s1 = makeStudent("Micheal","Jackson",5,"45 aveneue street","Manhatten","Gujarat","me@gmail.com",7,332,"CHE");
  // struct Node *h1,*e1,*h2,*e2;
  // h1->Stu = S[0];
  // h1->next = e1;
  // e1->Stu = S[1];
  // e1->next = NULL;
  ...
  // e2->Stu = S[2];
  // e2->next = NULL;
  struct Student sl1[] = {S[0], S[1]};
  struct Student sl2[] = {S[1], S[2]};
  struct Node *h1 = create(sl1, 2);
  struct Node *h2 = create(sl2, 2);
